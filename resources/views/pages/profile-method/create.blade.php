...
<x-app-layout>
    ...
    <div class="grid grid-cols-1 bg-white rounded-2xl shadow-sm p-6">
        ...
        <form action="{{ route('method-profile.store') }}" method="POST">
            @csrf
            <!-- Section -->
            ...
            <div
                ...
                <div class="sm:col-span-9">
                    <select for="af-submit-application-aspek" name="id_alternatif"
                ...
                @foreach ($kriteria->groupBy('id_kriteria') as $id_kriteria => $kriteriaGroup)
                    ...
                    @if ($kriteriaGroup->first()->kriteria_name == 'Keahlian Utama')
                        <div class="sm:col-span-9">
                            <select multiple id="af-submit-application-kriteria-{{ $id_kriteria }}"
                    ...
                    "extraMarkup": "<div class=\"absolute top-1/2 end-3 -translate-y-1/2\"><svg class=\"shrink-0 size-3.5 text-gray-500 \" xmlns=\"http://www.w3.org/2000/svg\" width=\"24\" height=\"24\" viewBox=\"0 0 24 24\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"2\" stroke-linecap=\"round\" stroke-linejoin=\"round\"><path d=\"m7 15 5 5 5-5\"/><path d=\"m7 9 5-5 5 5\"/></svg></div>"
                                ...
                                @foreach ($subkriteria->where('id_kriteria', $id_kriteria) as $subkriteriaItem)
                                    <option value="{{ $subkriteriaItem->id_subkriteria }}"
                                        @if (is_array(old("kriteria.$id_kriteria")) && in_array($subkriteriaItem->id_subkriteria, old("kriteria.$id_kriteria"))) selected @endif>
                    ...
                    @else
                        <div class="sm:col-span-9">
                            <select id="af-submit-application-kriteria-{{ $id_kriteria }}"
                                ...
                                @foreach ($subkriteria->where('id_kriteria', $id_kriteria) as $subkriteriaItem)
                                    <option value="{{ $subkriteriaItem->id_subkriteria }}"
                                        @if ($subkriteriaItem->id_subkriteria == old("kriteria.$id_kriteria", $kriteriaGroup->first()->nilai ?? null)) selected @endif>
                                        {{ $subkriteriaItem->nilai->value }} -
                                        ({{ $subkriteriaItem->subkriteria_name }})
                                    </option>
                                @endforeach
